import { YgoProDeckImageSourceAdapter } from '../../../../../context/card/infrastructure/external/ygoprodeck-image-source';
...
const buildImageBuffer = (): Buffer => Buffer.from([0xff, 0xd8, 0xff, 0xe0, 0x00, 0x10, 0x4a, 0x46, 0x49, 0x46]);
...
describe('YgoProDeckImageSourceAdapter', () => {
  ...
  it('returns buffer on successful fetch', async () => {
    ...
    fetchSpy = jest.spyOn(global, 'fetch').mockResolvedValue(buildImageResponse(imageBuffer, 200));

    const source = new YgoProDeckImageSourceAdapter(buildLoggerMock());
    const result = await source.fetchImage('46986414', CardImageVariant.create('normal'));

    expect(result).toEqual(imageBuffer);
    expect(fetchSpy).toHaveBeenCalledWith(
      'https://example.com/images/cards/46986414.jpg',
      expect.objectContaining({ headers: { Accept: 'image/jpeg,image/*,*/*' } }),
    );
  });

  it('returns null on 404', async () => {
    fetchSpy = jest.spyOn(global, 'fetch').mockResolvedValue(buildImageResponse(Buffer.alloc(0), 404));

    const source = new YgoProDeckImageSourceAdapter(buildLoggerMock());
    const result = await source.fetchImage('99999999', CardImageVariant.create('normal'));

    expect(result).toBeNull();
  });

  it('throws CardDomainProcessError on 500', async () => {
    fetchSpy = jest.spyOn(global, 'fetch').mockResolvedValue(buildImageResponse(Buffer.alloc(0), 500));

    const source = new YgoProDeckImageSourceAdapter(buildLoggerMock());

    await expect(source.fetchImage('46986414', CardImageVariant.create('normal'))).rejects.toThrow(
      CardDomainProcessError,
    );
  });

  it('throws CardDomainProcessError on non-OK non-404 status', async () => {
    fetchSpy = jest.spyOn(global, 'fetch').mockResolvedValue(buildImageResponse(Buffer.alloc(0), 403));
  ...
  it('uses correct URL for normal variant', async () => {
    fetchSpy = jest.spyOn(global, 'fetch').mockResolvedValue(buildImageResponse(buildImageBuffer(), 200));
