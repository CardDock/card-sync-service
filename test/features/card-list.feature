Feature: Card list with filters
  ...
  Scenario: Filter cards by type and attribute
    When I send a GET request to "/cards?type=Normal%20Monster&attribute=DARK"
    Then the response status should be 200
    And all returned cards should have type "Normal Monster" and attribute "DARK"

  Scenario: Filter cards by race
    When I send a GET request to "/cards?race=Spellcaster"
    Then the response status should be 200
    And all returned cards should have race "Spellcaster"
